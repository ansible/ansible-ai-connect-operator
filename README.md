...
`AnsibleAIConnect` can be configured to use existing `Secret`'s for both the `auth_config` and `model_config` configuration. Here is an [example](/docs/using-external-configuration-secrets.md)
